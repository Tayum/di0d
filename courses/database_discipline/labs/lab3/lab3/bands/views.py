...
search_api = api_module.SearchAPI()
...
def savefact(request):
    ...
    if request.GET.get('q', '') == "to_save":
        fact_info = db_management.read_fact_by_id(request.GET.get('id', '-1'))
        # (...) and if there was found a fact with such ID
        if fact_info['rows']:
            # Get the fact itself (its fields) without "description" of them
            # as far as we get a !LIST! with the only fact, we use [0] to access it
            ...
            fact_to_update['date_release'] = dict()
...
def api(request):
    ...
    if request.GET.get('search_mode', "") == "0" or request.GET.get('search_mode', "") == "1":
        object_to_return = search_api.artists__text_search(request)
    elif request.GET.get('search_mode', "") == "10":
        object_to_return = search_api.albums__enum(request)
    elif request.GET.get('search_mode', "") == "100":
        object_to_return = search_api.entries__number_search(request, "Artists")
    elif request.GET.get('search_mode', "") == "101":
        object_to_return = search_api.entries__number_search(request, "Albums")
    elif request.GET.get('search_mode', "") == "102":
        object_to_return = search_api.entries__number_search(request, "Songs")
